...
-export([hash/1, hash_dots/1, approximate_size/2]).
...
%% @doc generate a hash for an objects vclock and content dots
-spec hash_dots(riak_object()) -> integer().
hash_dots(Obj=#r_object{}) ->
    Vclock = vclock(Obj),
    erlang:phash(to_binary(v0, Vclock)).
